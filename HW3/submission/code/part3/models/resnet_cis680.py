    ...
    for x in cfg:
      if len(x) == 4: 
        if x[0] == 'M': # MaxPooling 
          ...
          mutltis =  n_in*n_in* in_channels*x[2]*n_out*n_out #  # DK · DK · M · N · DF · DF
          num_multi += mutltis 
          print('* *Resblock conv %d, DK, M, N, DF = %d %d %d %d, compute %d'%(num_conv, n_in,  in_channels, x[3], n_out, mutltis))
          n_in, j_in, r_in, start_in = n_out, j_out, r_out, start_out
          num_conv += 1

          n_out, j_out, r_out, start_out = get_rep_field(n_in, j_in, r_in, start_in, kernel_size=3, stride=1, padding=1)
          mutltis =  n_in*n_in* x[3]*x[3]*n_out*n_out #  # DK · DK · M · N · DF · DF
          num_multi += mutltis 
          print('* *Resblock conv %d, DK, M, N, DF = %d %d %d %d, compute %d'%(num_conv, n_in,  x[3], x[3], n_out, mutltis))
          n_in, j_in, r_in, start_in = n_out, j_out, r_out, start_out
          num_conv += 1

          n_out, j_out, r_out, start_out = get_rep_field(n_in, j_in, r_in, start_in, kernel_size=1, stride=1, padding=0)
          mutltis =  n_in*n_in* in_channels*x[3]*n_out*n_out #  # DK · DK · M · N · DF · DF
          num_multi += mutltis 
          print('* *Resblock conv %d, DK, M, N, DF = %d %d %d %d, compute %d'%(num_conv, n_in,  in_channels, x[3], n_out, mutltis))
          n_in, j_in, r_in, start_in = n_out, j_out, r_out, start_out
          num_conv += 1
