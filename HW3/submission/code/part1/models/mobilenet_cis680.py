    ...
    for x in cfg:
      if len(x) == 4: 
        if x[0] == 'M': # maxpooling 
          ...
          mutltis =  x[1]*x[1]* in_channels*n_out*n_out + in_channels*x[3]*n_out*n_out # DK · DK · M · DF · DF + M · N · DF · DF
          num_multi += mutltis 
          print('* conv %d, K, M, N, DF = %d %d %d %d, compute %d'%(num_conv, x[1],  in_channels, x[3], n_out, mutltis))
