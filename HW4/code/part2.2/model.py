...
flags = tf.app.flags
flags.DEFINE_string("logdir", home_dir+'/logs/2.2/', "Directory to save logs")
flags.DEFINE_string("sampledir", home_dir+'/out/2.2/', "Directory to save samples")
...
flags.DEFINE_boolean("max_steps", int(60000), "Max number of training steps")
FLAGS = flags.FLAGS
...
def DCGAN_main(dataset):
  ...
  for step in range(start_step, 2 if FLAGS.debug else start_step + FLAGS.max_steps):    
    ...
    if step % FLAGS.interval_plot == 0:
      # Save samples
      if FLAGS.sampledir:
          samples = 16 if FLAGS.batch_size >= 16 else FLAGS.batch_size 
          if FLAGS.fix_sample:
            z2 = Z2
          else:
            z2 = np.random.uniform(-1.0, 1.0, size=[FLAGS.batch_size, z_dim]).astype(np.float32)
          images = sess.run(g_model, feed_dict={z: z2})
          images = np.reshape(images, [FLAGS.batch_size, 64, 64]).astype(np.float32)
          images = (images + 1.) / 2.
          scipy.misc.imsave(FLAGS.sampledir + 'sample_%d.png'%step, merge(images[0:samples,...], [int(math.sqrt(samples))] * 2))
    if step % 1000 == 0:
      # save model
      if not FLAGS.debug:
