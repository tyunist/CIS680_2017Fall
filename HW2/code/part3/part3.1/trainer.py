...
class Trainer(object):
  ...
  def __init__(self, config, img_np, correct_labels_np, wrong_labels_np, original_confidence_np = None, img_index= 0, test_valid=True):
    self.config = config
    ...
    self.img_np = img_np
    self.correct_labels_np = correct_labels_np 
    self.wrong_labels_np  = wrong_labels_np
    self.original_confidence_np = original_confidence_np
    ...
    self.x_updated = tf.placeholder(tf.float32, [None, 32, 32, 3], name='updated-x-input') 
    self.x = tf.placeholder(tf.float32, [None, 32, 32, 3], name='original-x-input') 

 
    self.correct_labels = tf.placeholder(tf.int64, [None], name='train-correct_label') 
    self.wrong_labels = tf.placeholder(tf.int64, [None], name='train-wrong_label') 
    if not test_valid:
      self.build_model()
    else:
  ...
  def train_adv_image(self, adv_img_number, adv_img_name, adv_img_f, result_fig_name):
    ...
    try:
      for step in trange(self.start_step, self.max_step):
        ...
        source_acc = confidence[img_index, self.correct_labels_np[img_index]]
        target_acc = confidence[img_index, self.wrong_labels_np[img_index]]
        ...
        print('x:', np.sum(x))
        print('>>> Correct %d  Wrong %d'%( self.correct_labels_np[img_index] , self.wrong_labels_np[img_index]))
        print('>>> conf of correct %.4f conf of wrong %.4f total prob %.1f'%(source_acc, target_acc, np.sum(confidence)))
        print('update_rate:', update_rate)
        sys.stdout.flush()
        # print('...First elements of correct_labels_np:', self.correct_labels_np[0])
        ...
        if np.max(confidence[:, self.wrong_labels_np[img_index]]) > .99:
          img_index = np.argmax(confidence[:, self.wrong_labels_np[img_index]])
          print('>>> Wrong confidence > .99. Breaking...')
          source_acc = confidence[img_index, self.correct_labels_np[img_index]]*100 
          target_acc = confidence[img_index, self.wrong_labels_np[img_index]]*100
          start_source_acc = self.original_confidence_np[img_index, self.correct_labels_np[img_index]]*100
          start_target_acc = self.original_confidence_np[img_index, self.wrong_labels_np[img_index]]*100
          print('Index: %d with right %3.f wrong %3.1f '%(img_index, source_acc, target_acc))
    ...
    plt.subplot(131)
    plt.imshow(cv2.cvtColor(self.img_np[img_index].reshape([32,32,3]).astype(np.uint8), cv2.COLOR_BGR2RGB )) 
    plt.axis('off')
    plt.title('Original. source: (%3.1f %%) %d, target: (%3.1f %%) %d' % (start_source_acc,int(self.correct_labels_np[img_index]),\
                                                              start_target_acc, int(self.wrong_labels_np[img_index]) ) , fontsize=25)

    plt.subplot(132)
    plt.imshow(x_grad_updated[img_index].reshape([32,32,3]))
    plt.title('Delta (%.2f)' % (np.mean(x_grad_updated)), fontsize=25)
    plt.axis('off')

    plt.subplot(133)
    x_adversarial = cv2.cvtColor(x_updated[img_index].reshape([32,32,3]).astype(np.uint8), cv2.COLOR_BGR2RGB)
    plt.imshow(x_adversarial)
    # plt.imshow(x_updated.reshape([32,32,3]))
    plt.axis('off')
    plt.title('Adversarial source: (%3.1f %%) %d, target: (%3.1f %%) %d' % (source_acc, int(self.correct_labels_np[img_index]),\
                                                                  target_acc, int(self.wrong_labels_np[img_index])), fontsize=25)

    
  
    plt.savefig(result_fig_name)
    # plt.show()
    adv_img_f.write("{0} {1}\n".format(adv_img_name, int(self.correct_labels_np[img_index])))
  ...
  def build_model(self, first_time=False):
    ...
    self.c_loss, feat, self.accuracy, self.c_var, self.confidence = self.cnn_model_set[self.cnn_model_name](
      x, self.wrong_labels, self.c_num, self.batch_size, is_train=True, reuse=False,\
    ...
    # Gradient w.r.t x 
    print('...original grad shape:', tf.gradients(self.c_loss, x, self.c_loss)[0].get_shape())
    self.x_grad = tf.reshape(tf.gradients(self.c_loss, x)[0],  shape= [self.batch_size, 32, 32, 3]) #, self.c_loss)
  ...
  def build_test_model(self):
    ...
    loss, _, self.test_accuracy, var, self.test_confidence = self.cnn_model_set[self.cnn_model_name](
      x, self.correct_labels, self.c_num, self.batch_size_test, is_train=False, reuse=False, \
      make_grad_vanish=self.make_grad_vanish, resolve_grad_vanish=self.resolve_grad_vanish)
